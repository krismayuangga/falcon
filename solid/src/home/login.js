...
// alert("The URL of this page is: " + window.location.path);
sessionStorage.setItem("C_URL",window.location)
...
$("#oauthaccountsign").click(function () {
  window.open(
    `https://trade.kinerja.io/register`,
    "_blank"
  );
});
...
function postLogin() {
  ...
  let selectedaccount = document.getElementById('accountSelect').options[document.getElementById('accountSelect').selectedIndex].text;
  ...
  document.getElementById('notifyme').insertAdjacentHTML("afterbegin",
    '<p style="color:#8c01a0">Account switched to ' + selectedaccount + ', reauthorizing!</p>'
