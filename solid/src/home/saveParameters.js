function saveParametersOnClick() {
   ...
   // tokenToBeUsed = document.getElementById('setToken').value;
    ...
    if ($('#Stratdropdown').dropdown('get value') == "") {} else {
        autoStrat = $('#Stratdropdown').dropdown('get value');
    }

    if ($('#Stratdropdowntype').dropdown('get value') == "") {} else {
        autoStrattype = $('#Stratdropdowntype').dropdown('get value');
    }



    if ($('#MMdropdown').dropdown('get value') == "") {} else {
        MGStyle = $('#MMdropdown').dropdown('get value');
    }
    if ($('#Marketdropdown').dropdown('get value') == "") {} else {
    ...
    if (runrun === false || autoTrade === false) {
        ...
        message = 'Ready to place manual trade. Trade duration set to ' + convertedDuration +
            ' seconds';

        console.log('saved', takeProfit, stopLoss, trailingStop, autoThresh, MGReset, ManualtradeDurationInt, ManualtradeDurationUnit, autoStrat)
