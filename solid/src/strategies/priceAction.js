    ...
    if (runrun && autoTrade && tradeInProgress === false) {
        ...
        if (Close > Open && CandlePercent > 0.7) {
            ...
            if (seconds > candleEnding || seconds < candleStarting) {
                ...
                document.getElementById('notifyme').insertAdjacentHTML("afterbegin", '<p>' + time + ': : priceAction CALL triggered </p>');
                callOrPut = 'CALL';
                //  tradeInProgress: true
                trade();
            }
        ...
        if (Close < Open && CandlePercent > 0.7) {
            ...
            if (seconds > candleEnding || seconds < candleStarting) {
                ...
                document.getElementById('notifyme').insertAdjacentHTML("afterbegin", '<p>' + time + ': : priceAction PUT triggered </p>');
                callOrPut = 'PUT';
                //  tradeInProgress: true
                trade();
            }
